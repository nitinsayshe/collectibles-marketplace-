...
GOOGLE_DRIVE_REDIRECT_URI=http://localhost:3001/api/google-drive/callback
...
Browser ──▶ Vercel (Next.js, marketplace-frontend/)
            ...
            Render (NestJS, marketplace-backend/) ──▶ MongoDB Atlas
              │
              └─▶ Google OAuth (Drive image storage)
```
...
3. **Render (backend)**:
   ...
   - `JWT_SECRET` and `TOKEN_ENCRYPTION_KEY` are auto-generated by the blueprint. Fill in the rest (`MONGODB_URI`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`) in the Render dashboard.
   - After first deploy, copy the assigned URL (e.g. `https://marketplace-backend-xxxx.onrender.com`) into `BACKEND_PUBLIC_URL` and `GOOGLE_DRIVE_REDIRECT_URI` (`<that URL>/api/google-drive/callback`), then redeploy.
...
4. **Vercel (frontend)**: New Project → import the repo → set **Root Directory** to `marketplace-frontend` (Next.js is auto-detected, no config file needed). Set env vars:
   ```
   NEXT_PUBLIC_API_URL=https://<render-backend-url>/api
   NEXT_PUBLIC_SOCKET_URL=https://<render-backend-url>
   ```
5. **Update Render's `FRONTEND_URL`** to the Vercel domain (e.g. `https://your-app.vercel.app`) once it's known — the backend uses it for both REST CORS ([main.ts](../marketplace-backend/src/main.ts)) and Socket.io CORS ([chat.gateway.ts](../marketplace-backend/src/gateway/chat.gateway.ts)).
6. **Google Cloud Console** (OAuth client used for Google Drive image storage):
   - Credentials → the OAuth client → Authorized redirect URIs → add the production `GOOGLE_DRIVE_REDIRECT_URI` from step 3 (keep the localhost one too, for local dev).
   - OAuth consent screen → while in **Testing** mode, add every Google account that needs to use image upload as a **Test user** (max 100), otherwise they'll hit `Error 403: access_denied`. Submit for verification only if this needs to be open to arbitrary public Google accounts.
...
- S3 image upload (currently uses URL input)
